...
export const employeesSyncs = pgTable(
    'employees_syncs',
    {
        employee_sync_id                   : text()
            .notNull()
            .unique(),
        employee_sync_organization_id      : text()
            .notNull(),
        employee_sync_employee_id          : text()
            .notNull(),
        employee_sync_last_synced_timestamp: integer()
            .notNull()
    },
    (table) => {
        return {
            ...
            organizationIdIndex: index('employee_sync_organization_id_idx')
                .on(table.employee_sync_organization_id),
            employeeIdIndex    : index('employee_sync_employee_id_idx')
                .on(table.employee_sync_employee_id)
        }
    }
)
