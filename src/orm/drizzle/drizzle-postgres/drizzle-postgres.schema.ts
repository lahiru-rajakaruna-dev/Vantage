...
export const SchemaInsertEmployeeActivity = createInsertSchema(
    ...
    .superRefine((data, ctx) => {
        switch (data.employee_activity_type) {
            case EActivityType.SALE_INITIALIZED:
            case EActivityType.SALE_CLOSED:
            case EActivityType.SALE_PROSPECTED:
            case EActivityType.CLIENT_ADDED:
            case EActivityType.CLIENT_UPDATED:
            case EActivityType.PAYMENT_ADDED:
            case EActivityType.SALARY_PAID:
            case EActivityType.CHECK_IN:
            case EActivityType.CHECK_OUT:
            case EActivityType.TRAVEL_STARTED:
            case EActivityType.TRAVEL_ENDED:
                if (!data.employee_activity_latitude || !data.employee_activity_longitude) {
                    ctx.addIssue({
                                     code   : z.ZodIssueCode.custom,
                                     message: `Location (latitude/longitude) is required for ${ data.employee_activity_type }`,
                                     path   : [ 'employee_activity_latitude' ]
                                 });
                }
                break;
            
            case EActivityType.LEAVE_APPROVED:
            case EActivityType.LEAVE_REJECTED:
            case EActivityType.EXPENSE_SUBMITTED:
            case EActivityType.EXPENSE_APPROVED:
            case EActivityType.EXPENSE_REJECTED:
            case EActivityType.DOCUMENT_UPLOADED:
        }
        
        const now = Math.floor(Date.now() / 1000);
        if (data.employee_activity_timestamp > now + 300) { // Allow 5 minutes grace for clock differences
            ctx.addIssue({
                             code   : z.ZodIssueCode.custom,
                             message: 'Activity timestamp cannot be in the future',
                             path   : [ 'employee_activity_timestamp' ]
                         });
        }
    });
...
export const SchemaSelectEmployeeActivity = createSelectSchema(
    ...
    .extend({
                ...
                employee_activity_message       : z.string(),
                employee_activity_latitude      : z.number().optional(),
                employee_activity_longitude     : z.number().optional(),
